...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " suqa-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  suqa-tx [options] <hex-tx> [commands]  Update hex-encoded suqa transaction\n" +
            "or:     suqa-tx [options] -create [commands]   Create hex-encoded suqa transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded suqa transaction
