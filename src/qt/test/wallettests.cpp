...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<SUQAAmountField*>("payAmount")->setValue(amount);
...
//
//     src/qt/test/test_suqa-qt -platform xcb      # Linux
//     src/qt/test/test_suqa-qt -platform windows  # Windows
//     src/qt/test/test_suqa-qt -platform cocoa    # macOS
void TestGUI()
{
    ...
    // Amount input
    SUQAAmountField* amountInput = receiveCoinsDialog.findChild<SUQAAmountField*>("reqAmount");
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QVERIFY(paymentTextList.at(1).indexOf(QString("URI: suqa:")) != -1);
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'test_suqa-qt -platform cocoa' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI();
